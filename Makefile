...
PKG_SUBDIRS=$(shell go list ./... | grep -v /vendor/ | sed -r 's|github.com/elixirhealth/entity/||g' | sort)
...
acceptance:
	...
	@mkdir -p artifacts
	@go test -tags acceptance -v github.com/elixirhealth/entity/pkg/acceptance 2>&1 | tee artifacts/acceptance.log
...
build-static:
	...
	@./vendor/$(SERVICE_BASE_PKG)/scripts/build-static deploy/bin/entity
...
docker-image:
	...
	@docker build --rm=false -t gcr.io/elixir-core-prod/entity:snapshot deploy
...
proto:
	@echo "--> Running protoc"
	@protoc pkg/entityapi/entity.proto -I. -I vendor/ --go_out=plugins=grpc:.
